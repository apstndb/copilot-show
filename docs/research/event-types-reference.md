...
| --- | --- | --- | --- |
| `session.start`, `session.resume` | Start a new local segment | `id`, `parentId`, `data.eventCount` on resume | `session.resume` is a continuity checkpoint. Segment-local turn tracking resets here. |
| `session.shutdown` | Close a segment and summarize it | `data.totalPremiumRequests`, `data.modelMetrics`, `data.currentModel`, `data.totalApiDurationMs`, `data.codeChanges.*` | This is the authoritative source for closed-segment premium billing and the best local segment summary. |
